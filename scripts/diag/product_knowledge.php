...
$s = db()->prepare("SELECT kc.id, kc.source_type, kc.source_id, kf.filename, substr(kc.chunk_text,1,900) t
                    FROM knowledge_chunks kc LEFT JOIN knowledge_files kf ON kf.id = kc.source_id AND kc.source_type='file'
                    WHERE upper(kc.chunk_text) LIKE ? AND kc.source_type != 'standard' LIMIT 6");
$s->execute([$like]);
