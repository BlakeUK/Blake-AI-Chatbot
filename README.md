...
| **Files / RAG** | Upload PDFs, images, DOCX, CSV etc. Gemini extracts text, chunks it, indexes it. Optionally tag a batch with a category (e.g. "TV Aerials", "CCTV & Security") to boost its relevance when a customer is on a product page in that range. The "Pages" sub-tab indexes a live website page's actual body content (not just its title/description) and can be pointed at a sitemap for a one-off scan or, under **Scheduled Site Refresh**, a daily background re-crawl that keeps content in sync automatically. A byte-identical file re-upload is skipped automatically (no wasted Gemini call); anything that merely looks very similar to existing content is surfaced under **Possible Duplicates** for you to review — never deleted automatically. |
